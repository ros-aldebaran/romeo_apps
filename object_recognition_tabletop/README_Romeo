 ...
 * To install romeo_sensors on your computer: 
```
...
sudo apt-get install ros-indigo-ecto* ros-indigo-sql-database
 ...
 *  Run romeo_sensors/camera to publish images for tabletop:
```
...
rosrun object_recognition_core detection -c `rospack find object_recognition_tabletop`/conf/detection.romeo.object.ros.ork>
```
...
II.1 Prepare romeo_robot and tabletop
--------------------------------------

In order to use it on your robot, you need to compile the package romeo_robot and tabletop in a naoqi environment. Compiling any ROS package in naoqi 
...
Once you have your nao_camera and tabletop compiled and installed on your robot, you need to modify the detection config file in order to provide the right ip of the database. The config file is *tabletop/conf/detection.romeo.object.ros.ork*, you need to modify the root parameter of db, as following: 
